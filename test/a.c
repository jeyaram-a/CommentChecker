...
**/
VOID
pal_gic_create_info_table(gic_info_table_t *GicTable)
{
  EFI_ACPI_6_1_GIC_STRUCTURE    *Entry = NULL;
  gic_info_entry_t                *GicEntry = NULL;
  UINT32                         Length= 0;
  UINT32                         TableLength;
  ...
  do {
    ...
    if (Entry->Type == EFI_ACPI_6_1_GIC) {
      if (Entry->PhysicalBaseAddress != 0) {
        ...
        pal_print(ACS_LOG_INFO, "\n        GIC CPUIF base %x", GicEntry->base);
        GicEntry++;
      }
    }
...
UINT32
pal_gic_free_interrupt(UINT32 int_id)
{
}
